...
<div style="max-width:600px;margin:24px auto;padding:0 16px;">
    <div style="background:#fff;border-radius:12px;padding:24px;">
        <h2 style="font-size:20px;margin-bottom:20px;">✏️ Editar producto</h2>
        ...
        <div style="margin-bottom:14px;">
            <label style="display:block;font-size:13px;color:#555;margin-bottom:4px;">Actualizar foto (opcional)</label>
            ...
            <img id="preview-foto" src="" alt=""
                style="display:none;width:100%;max-height:200px;object-fit:contain;border-radius:8px;margin-top:8px;">
        </div>

        <div style="margin-bottom:14px;">
            <label style="display:block;font-size:13px;color:#555;margin-bottom:4px;">Nombre *</label>
            <input type="text" id="f-nombre" value="{{ $producto->nombre }}"
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
            ...
            <div>
                ...
                <input type="text" id="f-referencia" value="{{ $producto->referencia }}"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
            <div>
                ...
                <select id="f-categoria"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
                    @foreach(['Tornillería','Herramientas','Construcción','Eléctrico','Plomería','Pintura','Seguridad','General'] as $cat)
            ...
            <div>
                ...
                <select id="f-unidad"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
                    @foreach(['unidad','metro','kilo','litro','bolsa','caja'] as $u)
                    <option value="{{ $u }}" {{ $producto->unidad === $u ? 'selected' : '' }}>{{ ucfirst($u) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">
            <div>
                ...
                <input type="number" id="f-precio-compra" value="{{ $producto->precio_compra }}"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
            </div>
            <div>
                <label style="display:block;font-size:13px;color:#555;margin-bottom:4px;">Precio venta (COP) *</label>
                <input type="number" id="f-precio-venta" value="{{ $producto->precio_venta }}"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px;">
            <div>
                ...
                <input type="number" id="f-stock" value="{{ $producto->stock }}"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
            </div>
            <div>
                <label style="display:block;font-size:13px;color:#555;margin-bottom:4px;">Stock mínimo</label>
                <input type="number" id="f-stock-minimo" value="{{ $producto->stock_minimo }}"
                    style="width:100%;padding:12px;font-size:16px;border:2px solid #ddd;border-radius:8px;">
            </div>
        </div>
...
async function guardar() {
    ...
    const res = await fetch('/inventario/{{ $producto->id }}/actualizar-producto', {
        method: 'POST',
        headers: {
