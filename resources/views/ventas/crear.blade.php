...
<head>
    ...
    <title>Nueva Venta - POS Ferretero</title>
...
// Refresca el token CSRF desde el servidor antes de cada venta
async function refrescarCsrf() {
    const res = await fetch('/up', { credentials: 'same-origin' });
    // El token en el meta tag se actualiza automáticamente con la sesión activa
...
async function procesarVenta() {
    ...
    try {
        ...
        // Si el token expiró recargar página automáticamente
        if (res.status === 419) {
